...
func (w *Worker) Run(ctx context.Context) {
	...
	for {
		...
		obj, action, userID := w.processMessage(ctx, msg, rh)
		...
		if action == Transfer {
			processCh <- struct{}{}
			go w.startMigration(ctx, obj, rh, &userID, destKey, transferCh, processCh)
		}
		if action == Delete {
			processCh <- struct{}{}
			go w.startDelete(ctx, obj, rh, &userID, destKey, processCh)
		}
	}
}
...
func (w *Worker) processMessage(ctx context.Context, msg, rh *string) (obj *Object, action int, userID string) {
	...
	if strings.Contains(*msg, `"eventSource":`) {
		...
		// Get the principalId of the SQS message for the data two way synchronization
		_userID := event.Records[0].UserIdentity.PrincipalId
		pos := strings.Index(_userID, ":")
		userID = _userID[pos+1 : pos+22]
...
func (w *Worker) startMigration(ctx context.Context, obj *Object, rh, userId *string, destKey *string, transferCh chan struct{}, processCh <-chan struct{}) {
	...
	if obj.Size <= int64(w.cfg.MultipartThreshold*MB) {
		res = w.migrateSmallFile(ctx, obj, userId, destKey, transferCh)
	} else {
		res = w.migrateBigFile(ctx, obj, userId, destKey, transferCh)
	}
...
func (w *Worker) startDelete(ctx context.Context, obj *Object, rh, userId *string, destKey *string, processCh <-chan struct{}) {
	...
	// Judge whether the object is from the destination or not.
	if *userId == w.cfg.UserID {
		log.Println("Repeated delete job!")
	} else {
...
func (w *Worker) migrateSmallFile(ctx context.Context, obj *Object, userId *string, destKey *string, transferCh chan struct{}) *TransferResult {
	...
	// Judge whether the object is from the destination or not.
	if *userId == w.cfg.UserID {
		log.Println("Repeated transfer job!")
...
func (w *Worker) migrateBigFile(ctx context.Context, obj *Object, userId *string, destKey *string, transferCh chan struct{}) *TransferResult {
